...
void routing_manager_base::notify(service_t _service, instance_t _instance,
    ...
    std::shared_ptr<event> its_event3 = find_event(_service, _instance, _event);
    if (its_event3) {
    ...
    std::shared_ptr<event> its_event4 = find_event(_service, _instance, _event);
    if (its_event4) {
        auto serializer4 = get_serializer(_service, _instance, domain_4, true);  // domain_num2
        its_event4->set_payload(serializer4, _payload, domain_4, _force, _flush);
    } else {
        VSOMEIP_WARNING << "Attempt to update the undefined event/field ["
            << std::hex << _service << "." << _instance << "." << _event
            << "]";
